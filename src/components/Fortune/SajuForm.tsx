...
import { Loader2, Sparkles, Info, UserCircle } from 'lucide-react';
...
export default function SajuForm({ onResult }: Props) {
  ...
  const [jijiTime,   setJijiTime]   = useState('');
  const [autoFilled, setAutoFilled] = useState(false);

  // 로그인된 경우 프로파일 로드 후 폼 자동 입력
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const payload: SajuInput = { ...form, birth_jiji: jijiTime || undefined, birth_time: undefined };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* 프로파일 자동 입력 배너 */}
      {user && autoFilled && (
        <div className="flex items-center gap-2 text-xs text-violet-700 bg-violet-50 border border-violet-100 rounded-xl px-4 py-2.5">
      ...
      {user && !autoFilled && (
        <div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5">
          <UserCircle size={14} className="flex-shrink-0" />
          <Link to="/profile" className="underline font-medium text-indigo-600">프로파일</Link>에 사주 정보를 저장하면 다음에 자동 입력됩니다.
        </div>
      )}
            ...
            {(['female', 'male'] as const).map(g => (
              <button key={g} type="button"
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${
                  form.gender === g ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                }`}>
                {g === 'female' ? '여성' : '남성'}
              </button>
            ))}
      ...
      <div>
        ...
        {jijiTime && (
          <p className="mt-1.5 text-xs text-violet-600 flex items-center gap-1">
            <Info size={11} />{jijiTime}시로 시주(時柱)를 분석합니다
          </p>
        )}
      </div>
        ...
        <div className="flex gap-2">
          {[CY - 1, CY, CY + 1].map(y => (
            <button key={y} type="button"
              ...
              className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${
                form.year === y ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
              }`}>
